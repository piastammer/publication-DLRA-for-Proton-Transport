...
function ComputeTrafoMatrices(Q::Quadrature,Norder::Int,nPN::Int,Omega::Array{Float64,2})
	nB = size(Omega,1)
	O = zeros(nB,Norder)
	M = zeros(Norder,nB)
	for b=1:nB
		_, θ, ϕ = cartesian_to_spherical(Omega[b,:])
		# _, ϕ, θ = cartesian_to_spherical(Omega[b,:])
		#μ = sqrt(1-cos(θ)^2)
		μ = cos(θ)
		# μ = Omega[b,3] 
		# ϕ = acos(Omega[b,1]/(sqrt(1-Omega[b,3]^2)))
		# μ = 1;
		# ϕ = 0;
		P = computePlmx(μ,lmax=nPN,norm=SphericalHarmonics.Unnormalized())
		#Ylm = computeYlm(θ, ϕ; lmax=nPN, SHType = SphericalHarmonics.RealHarmonics())
		counter = 1;
		for l=0:nPN
			for m=-l:l
				O[b,counter] =  real_sph(μ,ϕ,l,m,P) #Ylm[(l,m)]#
				M[counter, b] = O[b,counter]
				counter += 1
			end
		end
	end
